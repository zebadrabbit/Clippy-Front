...
- Files: All documentation uses uppercase filenames with `.md` extension (e.g., `WORKER_SETUP.md`, `WIREGUARD.md`, `TIERS-AND-QUOTAS.md`).
...
### scripts/worker/
Worker-side deployment and management helpers.
- Purpose: Worker deployment scripts, retention policies, supervisor configs.
- Do here: Deploy and provision workers.
- Key files: `clippy-retain.sh`, `supervisord.conf`, `smoke-artifact.sh`, `deploy_gpu_worker.sh`.
...
## secrets/
Local secrets directory (currently unused).
- Purpose: Reserved for future secret storage needs.
- Note: Workers now use API key authentication (`WORKER_API_KEY` in .env) instead of SSH keys.
...
## See also
- Documentation in `docs/` directory (all files use uppercase names with `.md` extension)
