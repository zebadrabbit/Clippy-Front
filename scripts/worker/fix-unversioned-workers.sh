...
echo ""
echo "# Or as background process (logs go to instance/logs/worker.json):"
echo "nohup ./scripts/worker/start-celery-versioned.sh main \"celery\" 2 > /dev/null 2>&1 &"
echo ""
...
echo ""
echo "Note: All worker logs are now structured JSON in instance/logs/worker.json"
