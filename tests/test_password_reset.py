...
def get_user(user_id):
    """Helper to get User object from ID (test_user fixture returns ID)."""
...
class TestPasswordResetRequest:
    ...
    def test_forgot_password_sends_email_for_valid_user(self, client, test_user):
...
class TestPasswordReset:
    ...
    def test_reset_password_page_renders_with_valid_token(self, client, test_user):
        """Test reset password page loads with valid token."""
        # Generate token
        token = test_user.generate_password_reset_token()
        test_user.reset_token = token
        test_user.reset_token_created_at = datetime.utcnow()
        db.session.commit()
    ...
    def test_reset_password_rejects_invalid_token(self, client):
        ...
        response = client.get("/reset-password/invalid-token-xyz")
        assert response.status_code == 302
        # Follow redirect to see error message
        response = client.get(
            "/reset-password/invalid-token-xyz", follow_redirects=True
        )
        assert b"Invalid or expired" in response.data

    def test_reset_password_rejects_expired_token(self, client, test_user):
        """Test reset password rejects expired token."""
        # Generate token but set creation time to 2 hours ago
        token = test_user.generate_password_reset_token()
        test_user.reset_token = token
        test_user.reset_token_created_at = datetime.utcnow() - timedelta(hours=2)
        db.session.commit()

        response = client.get(f"/reset-password/{token}", follow_redirects=True)
        assert b"Invalid or expired" in response.data

    def test_reset_password_updates_password(self, client, test_user):
    ...
    def test_reset_password_requires_matching_passwords(self, client, test_user):
    ...
    def test_reset_password_requires_minimum_length(self, client, test_user):
    ...
    def test_reset_password_updates_timestamp(self, client, test_user):
...
class TestUserTokenMethods:
    ...
    def test_generate_password_reset_token_creates_unique_token(self, test_user):
        """Test token generation creates unique tokens."""
        token1 = test_user.generate_password_reset_token()
        token2 = test_user.generate_password_reset_token()
        assert token1 != token2
        assert len(token1) > 20  # Should be substantial length

    def test_verify_password_reset_token_finds_user(self, test_user):
    ...
    def test_verify_password_reset_token_respects_max_age(self, test_user):
        """Test token verification respects max_age parameter."""
        token = test_user.generate_password_reset_token()
        test_user.reset_token = token
        # Set token creation to 10 seconds ago
        test_user.reset_token_created_at = datetime.utcnow() - timedelta(seconds=10)
        db.session.commit()
...
class TestPasswordResetSecurity:
    ...
    def test_forgot_password_redirects_authenticated_users(self, client, test_user):
        """Test authenticated users are redirected from forgot password page."""
        # Log in
        client.post(
            "/login",
            data={"username_or_email": test_user.username, "password": "test_password"},
        )
    ...
    def test_reset_password_redirects_authenticated_users(self, client, test_user):
    ...
    def test_token_can_only_be_used_once(self, client, test_user):
    ...
    def test_old_password_invalid_after_reset(self, client, test_user):
        ...
        response = client.post(
            "/login",
            data={"username_or_email": test_user.username, "password": old_password},
            follow_redirects=True,
        )
        ...
        response = client.post(
            "/login",
            data={"username_or_email": test_user.username, "password": new_password},
            follow_redirects=True,
        )
        assert response.status_code == 200
